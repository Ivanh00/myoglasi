...
class ServiceCategoryManagement extends Component
{
    ...
    {
        $this->selectedCategory = ServiceCategory::find($categoryId);
    ...
    public function createCategory()
    {
        ...
        $nextSortOrder = ServiceCategory::whereNull('parent_id')->max('sort_order') + 1;
    ...
    public function createSubcategory($parentId)
    {
        ...
        $nextSortOrder = ServiceCategory::where('parent_id', $parentId)->max('sort_order') + 1;
    ...
    public function saveCategory()
    {
        $rules = [
            ...
            'editState.slug' => [
                ...
                'string',
                'max:255',
                Rule::unique('service_categories', 'slug')->ignore($this->selectedCategory?->id)
            ],
            'editState.description' => 'nullable|string',
            'editState.icon' => 'nullable|string|max:1000',
            'editState.parent_id' => 'nullable|exists:service_categories,id',
        ...
        if ($this->selectedCategory) {
            ...
            $message = 'Kategorija usluga uspešno ažurirana!';
        } else {
            // Kreiranje nove kategorije
            ServiceCategory::create($validated['editState']);
            $message = 'Kategorija usluga uspešno kreirana!';
        }
    ...
    {
        $this->selectedCategory = ServiceCategory::with(['children', 'services'])->find($categoryId);
        ...
        // Provera da li kategorija ima usluge
        $totalServices = $this->selectedCategory->services->count();
        if ($totalServices > 0) {
            session()->flash('error', 'Ne možete obrisati kategoriju koja ima ' . $totalServices . ' usluga! Prvo premestite ili obrišite usluge.');
            return;
        }
    ...
    public function deleteCategory($force = false)
    {
        if ($this->selectedCategory) {
            ...
            session()->flash('success', 'Kategorija usluga uspešno obrisana!');
        }
    }
...
PHP;
    ...
    public function render()
    {
        ...
        return view('livewire.admin.service-category-management', compact('categories', 'parentCategories'))
