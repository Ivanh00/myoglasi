    ...
    <div class="max-w-4xl mx-auto px-4">
        ...
        <div class="bg-white rounded-lg shadow-lg p-6 mb-8">
            <div class="text-center">
                <h1 class="text-3xl font-bold text-gray-900 mb-4">Vaš balans</h1>
                <div class="inline-flex items-center justify-center w-32 h-32 bg-gradient-to-r from-blue-500 to-green-500 rounded-full mb-4">
                    <span class="text-white text-3xl font-bold">{{ number_format(auth()->user()->balance, 0, ',', '.') }}</span>
                </div>
                    ...
                    @foreach($this->transactions as $transaction)
                        <div class="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                            ...
                            <div class="text-right">
                                <p class="font-bold {{ $transaction->type === 'credit_topup' ? 'text-green-600' : 'text-red-600' }}">
                                    {{ $transaction->type === 'credit_topup' ? '+' : '-' }}{{ number_format($transaction->amount, 0, ',', '.') }} RSD
                                </p>
                                <span class="text-xs px-2 py-1 rounded-full {{ 
                                    ...
                                    @switch($transaction->status)
                                        @case('completed') Završeno @break
                                        @case('pending') Na čekanju @break
                                        @case('failed') Neuspešno @break
                                        @case('cancelled') Otkazano @break
                                        @case('awaiting_verification') Čeka verifikaciju @break
