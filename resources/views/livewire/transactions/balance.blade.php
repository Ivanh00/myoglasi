        ...
        <div class="fixed inset-0 bg-slate-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
            <div class="relative top-20 mx-auto p-5 border w-full max-w-md shadow-lg rounded-md bg-white dark:bg-slate-800">
                <div class="mt-3">
                    ...
                    <div class="{{ auth()->user()->balance > 0 ? 'bg-sky-50 border-sky-200' : 'bg-red-50 border-red-200' }} p-3 rounded-lg mb-4 border">
                        <div class="flex items-center">
                            <i class="fas fa-wallet {{ auth()->user()->balance > 0 ? 'text-sky-600 dark:text-sky-300' : 'text-red-600 dark:text-red-300' }} mr-2"></i>
                            <span class="{{ auth()->user()->balance > 0 ? 'text-sky-900' : 'text-red-900' }} font-medium">
                    ...
                    <form wire:submit.prevent="transferCredit">
                        <div class="space-y-4">
                            <!-- Recipient Search -->
                            <div>
                                <label class="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-2">Kome šaljete kredit?</label>
                                <div class="relative">
                                    ...
                                    @if(!empty($userSearchResults))
                                        <div class="absolute z-10 mt-1 w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-md shadow-lg max-h-40 overflow-y-auto">
                                            @foreach($userSearchResults as $user)
                                                <button type="button" wire:click="selectRecipient({{ $user->id }})"
                                                    ...
                                                    <div class="w-8 h-8 rounded-full bg-slate-50 dark:bg-slate-7000 flex items-center justify-center text-white text-xs mr-3">
                            ...
                            <!-- Transfer Amount -->
                            <div>
                                <label class="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-2">Iznos za transfer</label>
                            ...
                            <!-- Optional Note -->
                            <div>
                                <label class="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-2">Napomena (opciono)</label>
