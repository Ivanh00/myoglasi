                ...
                <div class="w-60" x-data="{ open: false }" x-init="open = false">
                    <div class="relative">
                        <button @click="open = !open" type="button"
                            class="w-full px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 rounded-lg shadow-sm text-gray-700 dark:text-gray-200 text-sm text-left hover:border-gray-400 focus:outline-none focus:border-green-500 transition-colors flex items-center justify-between">
                            <span>
                                @if($selectedCategory)
                        ...
                        <div x-show="open" @click.away="open = false" x-transition
                            ...
                            <button @click="$wire.setCategory(''); open = false" type="button"
                                class="w-full px-3 py-2 text-left text-sm hover:bg-gray-50 dark:hover:bg-gray-600 rounded-t-lg {{ !$selectedCategory ? 'bg-green-50 dark:bg-gray-600 text-green-700 dark:text-gray-200' : 'text-gray-700 dark:text-gray-200' }}">
                                Sve kategorije
                            </button>
                            @foreach ($categories as $category)
                                <button @click="$wire.setCategory('{{ $category->id }}'); open = false" type="button"
                                    class="w-full px-3 py-2 text-left text-sm hover:bg-gray-50 dark:hover:bg-gray-600 flex items-center {{ $selectedCategory == $category->id ? 'bg-green-50 dark:bg-gray-600 text-green-700 dark:text-gray-200' : 'text-gray-700 dark:text-gray-200' }}">
                                    @if($category->icon)
    ...
    @if ($giveaways->count() > 0)
        <div class="space-y-4 mb-8">
            @foreach ($giveaways as $giveaway)
                <div class="bg-white dark:bg-gray-700 rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300 border-l-4 border-green-500">
                    <div class="flex flex-col md:flex-row">
                            ...
                            <div class="flex flex-col h-full">
                                <div class="flex-1">
                                    <div class="flex items-center mb-2">
                                        <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100 hover:text-green-600 transition-colors">
                                            {{ $giveaway->title }}
                                        </h3>
                        ...
                        <!-- Desna strana - akcije -->
                        <div class="md:w-48 md:min-w-48 p-4 border-t md:border-t-0 md:border-l border-gray-200 dark:border-gray-600 bg-green-50 dark:bg-gray-600">
