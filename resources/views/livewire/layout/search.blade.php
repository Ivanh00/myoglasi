@php
    $hasActiveFilters = !empty(request('query')) || !empty(request('city')) || !empty(request('category')) ||
                        !empty(request('subcategory')) || !empty(request('condition')) ||
                        !empty(request('price_min')) || !empty(request('price_max'));
@endphp
    ...
    <form action="{{ route('search.index') }}" method="GET">
        <div class="flex shadow-sm rounded-md">
            ...
            <button type="button" @click="open = true; restoreValues()"
                ...
                Detaljno
                @if($hasActiveFilters)
                    <span class="ml-1 w-2 h-2 bg-sky-600 rounded-full"></span>
                @endif
            </button>
            <button type="submit"
