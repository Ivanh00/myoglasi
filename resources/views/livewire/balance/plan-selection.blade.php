    ...
    <div class="max-w-4xl mx-auto px-4">
        <div class="bg-white rounded-lg shadow-lg p-6">
            <div class="flex items-center justify-between mb-6">
                <h1 class="text-2xl font-bold text-slate-900">Vaš plan</h1>
            ...
            <div class="mb-8">
                <div class="bg-sky-50 border border-sky-200 rounded-lg p-4">
                    <div class="flex items-center justify-between">
                        <div>
                            <h3 class="text-lg font-semibold text-sky-900">Trenutni plan</h3>
                        ...
                        <div class="text-right">
                            <p class="text-sky-700 text-sm">Trenutni kredit:</p>
                            <p class="text-sky-900 font-bold text-xl">{{ number_format(auth()->user()->balance, 0, ',', '.') }} RSD</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            @if(!auth()->user()->payment_enabled || true)
                ...
                <form wire:submit.prevent="purchasePlan">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                        ...
                        @if($planPrices['per_listing']['enabled'])
                            <div class="border rounded-lg p-6 {{ $selectedPlan === 'per_listing' ? 'border-sky-500 bg-sky-50' : 'border-slate-200' }} transition-all cursor-pointer" 
                                ...
                                <div class="text-center">
                                        ...
                                        <h3 class="text-lg font-semibold text-slate-900 mb-2">{{ $planPrices['per_listing']['title'] }}</h3>
                                        ...
                                        <p class="text-xs text-slate-500">{{ $planPrices['per_listing']['description'] }}</p>
                        ...
                        @if($planPrices['monthly']['enabled'])
                            <div class="border rounded-lg p-6 {{ $selectedPlan === 'monthly' ? 'border-green-500 bg-green-50' : 'border-slate-200' }} transition-all cursor-pointer"
                                 wire:click="$set('selectedPlan', 'monthly')">
                                <div class="text-center">
                                        <div class="w-16 h-16 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-4">
                                            <i class="fas fa-calendar-alt text-green-600 text-2xl"></i>
                                        </div>
                                        <h3 class="text-lg font-semibold text-slate-900 mb-2">{{ $planPrices['monthly']['title'] }}</h3>
                                        <div class="text-2xl font-bold text-green-600 mb-2">
                                            {{ number_format($planPrices['monthly']['price'], 0, ',', '.') }} RSD
                                        </div>
                                        <p class="text-sm text-slate-600 mb-4">na mesec</p>
                                        <p class="text-xs text-slate-500 mb-2">{{ $planPrices['monthly']['description'] }}</p>
                        ...
                        @if($planPrices['yearly']['enabled'])
                            <div class="border rounded-lg p-6 {{ $selectedPlan === 'yearly' ? 'border-purple-500 bg-purple-50' : 'border-slate-200' }} transition-all cursor-pointer"
                                 wire:click="$set('selectedPlan', 'yearly')">
                                <div class="text-center">
                                        <div class="w-16 h-16 bg-purple-100 rounded-full flex items-center justify-center mx-auto mb-4">
                                            <i class="fas fa-calendar text-purple-600 text-2xl"></i>
                                        </div>
                                        <h3 class="text-lg font-semibold text-slate-900 mb-2">{{ $planPrices['yearly']['title'] }}</h3>
                                        <div class="text-2xl font-bold text-purple-600 mb-2">
                                            {{ number_format($planPrices['yearly']['price'], 0, ',', '.') }} RSD
                                        </div>
                                        <p class="text-sm text-slate-600 mb-4">na godinu</p>
                                        <p class="text-xs text-slate-500 mb-2">{{ $planPrices['yearly']['description'] }}</p>
                    ...
                    <!-- Plan Comparison -->
                    <div class="mb-8">
                        <h3 class="text-lg font-semibold text-slate-900 mb-4">Poređenje planova</h3>
                ...
                <div class="mt-8 pt-8 border-t border-slate-200">
                    <h3 class="text-lg font-semibold text-slate-900 mb-4">Prednosti planova</h3>
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 text-sm">
                        <div class="space-y-2">
                            <h4 class="font-semibold text-sky-600">Plaćanje po oglasu</h4>
