        ...
        @if ($results->count() > 0)
            ...
            @if ($viewMode === 'grid')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    @foreach ($results as $listing)
                        @if(isset($listing->is_auction))
                            @include('livewire.components.view-auction-component', ['auction' => $listing->auction_data, 'viewMode' => 'grid'])
                        @elseif($listing instanceof \App\Models\Service)
                            @include('livewire.components.view-service-component', ['service' => $listing, 'viewMode' => 'grid'])
                        @else
                    ...
                    @if ($viewMode === 'grid')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-8">
                            @foreach ($endedAuctions as $auction)
                                <div class="bg-white dark:bg-slate-800 rounded-lg shadow-md overflow-hidden border-l-4 border-amber-700">
                                    <!-- Image -->
                                    <div class="relative h-48">
                                        @if ($auction->listing->images->count() > 0)
                                            <img src="{{ $auction->listing->images->first()->url }}"
                                        ...
                                        @else
                                            <div class="w-full h-full bg-slate-200 dark:bg-slate-600 flex items-center justify-center opacity-75">
