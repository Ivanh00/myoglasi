            ...
            <div class="relative" x-data="{ open: false }" x-init="open = false">
                ...
                <div x-show="open" @click.away="open = false" x-transition
                    ...
                    <button @click="$wire.set('selectedCategory', ''); open = false" type="button"
                        class="w-full px-3 py-2 text-left text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600 rounded-t-lg {{ !$selectedCategory ? 'bg-blue-50 text-blue-700' : '' }}">
                        Sve kategorije
                    </button>
                    @foreach ($categories as $category)
                        <button @click="$wire.set('selectedCategory', '{{ $category->id }}'); open = false" type="button"
                            class="w-full px-3 py-2 text-left text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600 flex items-center {{ $selectedCategory == $category->id ? 'bg-blue-50 text-blue-700' : '' }}">
                            @if($category->icon)
        ...
        @if($viewMode === 'grid')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-8">
                @foreach ($listings as $listing)
                    <div class="listing-card {{ $listing->isHighlighted() ? 'bg-yellow-100 dark:bg-yellow-900 border-2 border-yellow-400 dark:border-yellow-600' : 'bg-white dark:bg-gray-700 border-l-4 border-blue-500' }} rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300">
                        <!-- Image -->
            ...
            <div class="space-y-4 mb-8">
                @foreach ($listings as $listing)
                    <div class="listing-card {{ $listing->isHighlighted() ? 'bg-yellow-100 dark:bg-yellow-900 border-2 border-yellow-400 dark:border-yellow-600' : 'bg-white dark:bg-gray-700 border-l-4 border-blue-500' }} rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300">
                        ...
                        <div class="md:w-48 md:min-w-48 p-4 border-t md:border-t-0 md:border-l border-gray-200 dark:border-gray-600 {{ $listing->isHighlighted() ? 'bg-yellow-50 dark:bg-yellow-800' : 'bg-blue-50 dark:bg-gray-600' }}">
