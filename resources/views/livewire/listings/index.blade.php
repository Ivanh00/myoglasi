        ...
        @if($viewMode === 'grid')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-8">
                @foreach ($listings as $listing)
                    <div class="listing-card {{ $listing->isHighlighted() ? 'bg-amber-100 dark:bg-amber-900 border-2 border-amber-400 dark:border-amber-600' : 'bg-white dark:bg-slate-700 border-l-4 border-sky-500' }} rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300">
                        <!-- Image -->
                        ...
                        <div class="p-4">
                            ...
                            @auth
                                ...
                                @if($listing->user && $listing->user->total_ratings_count > 0)
                                    <a href="{{ route('user.ratings', $listing->user->id) }}" class="inline-flex items-center text-xs text-slate-600 dark:text-slate-300 mb-2 hover:text-sky-600 transition-colors">
            ...
            <div class="space-y-4 mb-8">
                @foreach ($listings as $listing)
                    <div class="listing-card {{ $listing->isHighlighted() ? 'bg-amber-100 dark:bg-amber-900 border-2 border-amber-400 dark:border-amber-600' : 'bg-white dark:bg-slate-700 border-l-4 border-sky-500' }} rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300">
                        ...
                        <div class="md:w-48 md:min-w-48 p-4 border-t md:border-t-0 md:border-l border-slate-200 dark:border-slate-600 {{ $listing->isHighlighted() ? 'bg-amber-50 dark:bg-amber-800' : 'bg-sky-50 dark:bg-slate-600' }}">
                            <div class="flex flex-col h-full justify-between">
                                <div class="flex items-center justify-between text-sm text-slate-500 dark:text-slate-400 mb-4">
                                    <div class="flex items-center">
                                        <svg class="w-4 h-4 mr-1 text-slate-700 dark:text-slate-200" fill="currentColor" viewBox="0 0 24 24">
                                            <path
                                        ...
                                        <span class="text-slate-700 dark:text-slate-200">{{ $listing->views ?? 0 }}</span>
                                    </div>
                                    <!-- Dodajte ovaj div za prikaz broja pratilaca -->
                                    <div class="flex items-center">
                                        <i class=""></i>
                                        <span class="text-slate-700 dark:text-slate-200">❤️ {{ $listing->favorites_count ?? 0 }}</span>
                                    </div>
                                </div>
