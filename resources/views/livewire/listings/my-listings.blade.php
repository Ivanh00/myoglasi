                        ...
                        <div class="px-6 py-2">
                            <div class="flex items-center">
                                    <div class="flex-shrink-0 h-10 w-10">
                                        ...
                                        @else
                                            <div
                                                class="h-10 w-10 rounded-lg bg-slate-200 flex items-center justify-center">
                                    ...
                                    <div class="ml-4">
                                        ...
                                        <div class="text-sm text-slate-500 dark:text-slate-400">{{ $listing->category->name }}</div>
                                    </div>
                            </div>
                        </div>
                        <!-- Cena Column -->
                        <div class="px-6 py-2">
                            <div class="text-sm text-slate-900 dark:text-slate-100 font-bold whitespace-nowrap">{{ number_format($listing->price, 2) }} RSD
                            </div>
                        </div>
                        <!-- Status Column -->
                        <div class="px-6 py-2">
                                <div class="flex flex-col">
                                    @if ($listing->isExpired() || $listing->status == 'expired')
                                        ...
                                        @if($listing->expires_at)
                                            <span class="text-xs text-slate-500 dark:text-slate-400">Istekao {{ $listing->expires_at->format('d.m.Y') }}</span>
                                        @endif
                                    @elseif ($listing->status == 'active')
                                        ...
                                        @if($listing->expires_at)
                                            ...
                                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full
                                                ...
                                                @else bg-slate-200 dark:bg-slate-700 text-slate-800 dark:text-slate-200
                                                @endif">
                                                @if($daysLeft > 1)
                                    ...
                                    @elseif($listing->status == 'pending')
                                        <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-amber-200 dark:bg-amber-800 text-amber-800 dark:text-amber-200">Na čekanju</span>
                                    @else
                                        <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-slate-200 dark:bg-slate-700 text-slate-800 dark:text-slate-200">{{ ucfirst($listing->status) }}</span>
                                    @endif
                            </div>
                        </div>
                        <!-- Datum Column -->
                        <div class="px-6 py-2 text-sm text-slate-500 dark:text-slate-400">
                            <div class="flex flex-col">
                                ...
                                @if($listing->renewed_at)
                                    <span class="text-xs text-sky-500">Obnovljen {{ $listing->renewed_at->format('d.m.Y') }}</span>
                                @endif
                                @if($listing->renewal_count > 0)
                                    <span class="text-xs text-slate-400">({{ $listing->renewal_count }}x obnovljen)</span>
                                @endif
                            </div>
                        </div>
                        <!-- Akcije Column -->
            ...
            @foreach ($listings as $listing)
                <div class="bg-white dark:bg-slate-800 border-l-4 border-sky-500 shadow rounded-lg overflow-hidden">
                        ...
                        <div class="flex items-start justify-between">
                            <div class="flex items-start flex-1">
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <p class="text-sm text-slate-500 dark:text-slate-400 mb-2">{{ $listing->category->name }}</p>
                    ...
                    <div class="p-4">
                        <!-- Status Section -->
                        <div class="mb-4">
                            <div class="text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">Status oglasa</div>
                            <div class="flex flex-col space-y-2">
                                ...
                                @elseif ($listing->status == 'active')
                                    ...
                                    @endif
                                @elseif($listing->status == 'pending')
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-amber-200 dark:bg-amber-800 text-amber-800 dark:text-amber-200 w-fit">Na čekanju</span>
                                @else
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-slate-200 dark:bg-slate-700 text-slate-800 dark:text-slate-200 w-fit">{{ ucfirst($listing->status) }}</span>
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <div class="space-y-1">
                                <div class="text-sm text-slate-900 dark:text-slate-100">Kreiran: {{ $listing->created_at->format('d.m.Y') }}</div>
                                @if($listing->renewed_at)
                                    <div class="text-xs text-sky-500">Obnovljen: {{ $listing->renewed_at->format('d.m.Y') }}</div>
                                @endif
                                @if($listing->renewal_count > 0)
                        ...
                        <div class="bg-slate-50 dark:bg-slate-700 rounded-lg p-4 mb-4">
                            <div class="space-y-2">
                                <div class="flex justify-between items-center">
                                    ...
                                    <span class="text-sm font-medium text-slate-900 dark:text-slate-100" x-text="selectedListing?.title || 'N/A'"></span>
                                </div>
                                <div class="flex justify-between items-center">
                                    <span class="text-sm text-slate-600 dark:text-slate-300">Kategorija:</span>
                                    <span class="text-sm font-medium text-slate-900 dark:text-slate-100" x-text="selectedListing?.category?.name || 'N/A'"></span>
                                </div>
                                <div class="flex justify-between items-center">
                                ...
                                <div class="flex justify-between items-center">
                                    ...
                                    <span class="text-sm font-medium" :class="{
                                        ...
                                        'text-slate-600 dark:text-slate-400': selectedListing?.status !== 'active' && selectedListing?.status !== 'expired'
