<div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
    ...
    <div>
            <div class="bg-white dark:bg-slate-800 rounded-lg shadow-lg overflow-hidden mb-6">
                <!-- Auction Header -->
                <div class="bg-gradient-to-r from-orange-600 via-amber-400 to-orange-600 dark:from-orange-800 dark:via-amber-600 dark:to-orange-800 text-white p-4">
                            ...
                            <div class="relative">
                                ...
                                <div class="grid grid-cols-4 gap-2">
                                    @foreach($auction->listing->images as $index => $image)
                                        <div class="cursor-pointer border-2 rounded-lg overflow-hidden {{ $index === 0 ? 'border-amber-500' : 'border-slate-200' }}">
                                            <img src="{{ $image->url }}" 
                    ...
                    <div>
                        ...
                        @auth
                            @if(auth()->id() === $auction->user_id)
                                <!-- Owner Controls -->
                                <div class="mb-6 p-6 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 rounded-lg">
                                    ...
                                    <div class="space-y-3">
                                        ...
                                        @elseif($auction->isActive())
                                            ...
                                            @else
                                                <div class="w-full p-3 bg-amber-50 dark:bg-amber-900 border border-amber-200 dark:border-amber-700 rounded-lg">
                                            ...
                                            @endif
                                        @else
                                            <div class="w-full p-3 bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-lg text-center">
                            ...
                            @elseif(auth()->id() !== $auction->user_id)
                                @if($auction->isActive())
                                    <div class="mb-6 p-6 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 rounded-lg">
                                    <h3 class="text-lg font-semibold text-slate-900 dark:text-slate-100 mb-4">Postavite ponudu</h3>
                                    
                                    @if(!$isAutoBid)
                                        <div class="mb-4">
                                            <label for="bidAmount" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">
                                    ...
                                    <div class="mb-4">
                                        <div class="flex items-center justify-between">
                                            <label class="flex items-center cursor-pointer">
                                                ...
                                                <span class="text-sm text-slate-700 dark:text-slate-300">Automatska ponuda</span>
                                            </label>
                                        ...
                                        @if($isAutoBid)
                                            <div class="mt-3 p-3 bg-sky-50 dark:bg-sky-900 border border-sky-200 dark:border-sky-700 rounded-lg">
                                                <div class="mb-3">
                                                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Maksimalna cena</label>
                                                ...
                                                <div class="text-xs text-sky-800 dark:text-sky-200 space-y-1">
                                                    <div class="flex items-start">
                                                        <i class="fas fa-info-circle text-sky-600 dark:text-sky-400 mt-0.5 mr-2 flex-shrink-0"></i>
                                                        <div>
                        ...
                        @if(!auth()->check() || auth()->id() !== $auction->user_id)
                            @if($auction->status === 'active' && $auction->starts_at->isFuture())
                                <div class="mb-6 p-6 bg-sky-50 dark:bg-sky-900 border border-sky-200 dark:border-sky-700 rounded-lg">
                                    ...
                                    <div class="space-y-2">
                                        ...
                                        @else
                                            <div class="text-center text-slate-600 dark:text-slate-400 text-sm mb-2">
                                                <a href="{{ route('login') }}" class="text-sky-500 hover:text-sky-700">Prijavite se</a> da biste koristili ove opcije
                                            </div>
                                        @endauth
                ...
                <div class="border-t border-slate-200 dark:border-slate-600 p-6 bg-white dark:bg-slate-800">
                    ...
                    <div class="text-slate-700 dark:text-slate-300 whitespace-pre-line">{{ $auction->listing->description }}</div>
                </div>
                    ...
                    <div class="bg-white dark:bg-slate-800 rounded-lg shadow-lg overflow-hidden mb-6">
                        ...
                        <div class="flex items-start">
                            ...
                            <div class="flex-1">
                                ...
                                <p class="text-slate-600 dark:text-slate-400 text-sm mb-2">Član od: {{ $auction->seller->created_at->format('m/Y') }}</p>
                                ...
                                @if ($auction->listing->contact_phone && $auction->seller->phone_visible && !$auction->seller->is_banned)
                                    <div class="flex items-center text-sm text-slate-600 dark:text-slate-400 mb-3">
                                        ...
                                        <a href="tel:{{ $auction->listing->contact_phone }}" class="text-slate-700 dark:text-slate-300 hover:text-green-600">
                                ...
                                @if($auction->seller->total_ratings_count > 0)
                                    <a href="{{ route('user.ratings', $auction->seller->id) }}" class="inline-flex items-center text-sm text-slate-600 dark:text-slate-400 hover:text-sky-600 dark:hover:text-sky-400 transition-colors">
                                        <span class="text-green-600 mr-2">😊 {{ $auction->seller->positive_ratings_count }}</span>
                                        <span class="text-amber-600 mr-2">😐 {{ $auction->seller->neutral_ratings_count }}</span>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-lg shadow-lg p-6">
            <div class="flex items-center justify-between mb-4">
                ...
                <button wire:click="$refresh" class="text-sm text-sky-600 dark:text-sky-400 hover:text-sky-800 dark:hover:text-sky-300">
            ...
            @if($auction->bids->count() > 0)
                <div class="space-y-3 max-h-96 overflow-y-auto">
                    @foreach($auction->bids()->orderBy('amount', 'desc')->orderBy('created_at', 'desc')->take(10)->get() as $bid)
                        <div class="flex items-center justify-between p-3 {{ $bid->is_winning ? 'bg-green-50 dark:bg-green-900 border border-green-200 dark:border-green-700' : 'bg-slate-50 dark:bg-slate-700' }} rounded-lg">
                            <div class="flex items-center">
                                <div class="w-8 h-8 rounded-full flex items-center justify-center mr-3">
                                    ...
                                    @else
                                        <div class="w-8 h-8 bg-slate-500 rounded-full flex items-center justify-center text-white font-medium text-xs">
                                ...
                                <div>
                                    ...
                                    <div class="text-xs text-slate-500 dark:text-slate-400">{{ $bid->created_at->format('d.m.Y H:i') }}</div>
                                </div>
                            </div>
                            <div class="text-right">
                                <div class="font-bold {{ $bid->is_winning ? 'text-green-600' : 'text-slate-900 dark:text-slate-100' }}">
            ...
            <div class="mt-12">
                ...
                <div class="space-y-4">
                    @foreach ($recommendedListings as $relatedListing)
                        <div class="listing-card bg-white dark:bg-slate-800 rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300
            ...
            <div class="px-6 py-5">
                ...
                <div class="border-t border-slate-200 dark:border-slate-600 pt-3">
                    <div class="flex items-center text-sm">
                        ...
                        <span class="ml-2 font-medium text-slate-900 dark:text-slate-100">{{ $auction->seller->name }}</span>
                        {!! $auction->seller->verified_icon !!}
                    </div>
                    <div class="flex items-center text-sm mt-1">
                        <i class="fas fa-map-marker-alt text-slate-400 mr-2"></i>
                        <span class="text-slate-600 dark:text-slate-300">Lokacija:</span>
                        <span class="ml-2 text-slate-700 dark:text-slate-200">{{ $auction->listing->location }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
                <div x-transition:enter="transition ease-out duration-300"
                    ...
                    <div class="px-6 py-5">
                        ...
                        <div class="bg-slate-50 dark:bg-slate-700 rounded-lg p-4 mb-4">
                            <div class="space-y-2">
                                <div class="flex justify-between items-center">
                                    ...
                                    <span class="text-sm font-medium text-slate-900 dark:text-slate-100">{{ $auction->listing->title }}</span>
                                </div>
                                <div class="flex justify-between items-center">
                                    <span class="text-sm text-slate-600 dark:text-slate-300">Trenutna cena:</span>
                                    <span class="text-sm font-medium text-slate-900 dark:text-slate-100">{{ number_format($auction->current_price, 0, ',', '.') }} RSD</span>
                                </div>
                                <div class="flex justify-between items-center">
                                    <span class="text-sm text-slate-600 dark:text-slate-300">Broj ponuda:</span>
                                    <span class="text-sm font-medium text-slate-900 dark:text-slate-100">{{ $auction->total_bids }}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        @elseif($auction->status === 'active' && $auction->starts_at->isFuture())
                            <div class="bg-sky-50 dark:bg-sky-900/20 border border-sky-200 dark:border-sky-700 rounded-lg p-3">
                                <div class="flex">
                                    <div class="flex-shrink-0">
