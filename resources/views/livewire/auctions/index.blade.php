                ...
                <div class="relative mb-2" x-data="{ open: false }" x-init="open = false">
                    ...
                    <div x-show="open" x-cloak @click.away="open = false" x-transition
                        ...
                        <button @click="$wire.set('selectedSubcategory', ''); open = false" type="button"
                            class="w-full px-3 py-2 text-left text-sm hover:bg-slate-50 dark:hover:bg-slate-600 rounded-t-lg {{ !$selectedSubcategory ? 'bg-amber-50 dark:bg-amber-900 text-amber-700 dark:text-amber-200' : 'text-slate-700 dark:text-slate-200' }}">
                            Sve podkategorije
                        </button>
                        @foreach ($subcategories as $subcategory)
                            <button @click="$wire.set('selectedSubcategory', '{{ $subcategory->id }}'); open = false"
                                type="button"
                                class="w-full px-3 py-2 text-left text-sm hover:bg-slate-50 dark:hover:bg-slate-600 {{ $selectedSubcategory == $subcategory->id ? 'bg-amber-50 dark:bg-amber-900 text-amber-700 dark:text-amber-200' : 'text-slate-700 dark:text-slate-200' }}">
                                {{ $subcategory->name }}
                            </button>
                        @endforeach
                    </div>
            ...
            <div class="flex gap-3">
                ...
                <div class="flex-1" x-data="{ open: false }" x-init="open = false">
                    <div class="relative">
                        ...
                        <div x-show="open" x-cloak @click.away="open = false" x-transition
                            class="absolute z-10 mt-1 w-full bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-lg shadow-lg">
                            <button @click="$wire.set('sortBy', ''); open = false" type="button"
                                class="w-full px-3 py-2 text-left text-sm text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-600 rounded-t-lg">
    ...
    @if ($auctions->count() > 0)
        @if ($viewMode === 'grid')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-8">
                @foreach ($auctions as $auction)
                    @include('livewire.components.view-auction-component', ['auction' => $auction, 'viewMode' => 'grid'])
                @endforeach
            </div>
        @endif
        ...
        @if ($viewMode === 'list')
            ...
            <div class="space-y-4 mb-8">
                @foreach ($auctions as $auction)
                    @include('livewire.components.view-auction-component', ['auction' => $auction, 'viewMode' => 'list'])
                @endforeach
            </div>
        @endif
                    ...
                    @foreach ($endedAuctions as $auction)
                        <div
                            ...
                            <div class="p-4">
                                ...
                                @auth
                                    <p class="text-sm font-bold text-slate-700 dark:text-slate-200 mb-1">
                                        {{ $auction->seller->name ?? 'Nepoznat korisnik' }}
                                        @if ($auction->seller)
