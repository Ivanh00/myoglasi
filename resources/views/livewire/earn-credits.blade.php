            ...
            <div class="bg-white dark:bg-slate-800 rounded-lg shadow-lg p-2 md:p-6 mb-8">
                <div class="text-center">
                    ...
                    <div class="mb-6">
                        ...
                        <p class="text-slate-600 dark:text-slate-400 mt-4">Klikni na dugme što više puta!</p>
                    </div>
        ...
        @if ($gameActive && $selectedGame === 'memory_game')
            <div class="bg-white dark:bg-slate-800 rounded-lg shadow-lg p-2 md:p-6 mb-8">
                <div class="text-center">
                    ...
                    <div class="mb-6">
                        <div class="text-lg font-bold text-green-600 dark:text-green-400 mb-2">Potezi:
                            {{ $memoryMoves }}</div>
                    </div>
                            ...
                            @for ($i = 0; $i < 12; $i++)
                                ...
                                <div wire:click="flipMemoryCard({{ $i }})"
                                    class="w-14 h-14 rounded-lg cursor-pointer transition-all transform hover:scale-105 flex items-center justify-center {{ $memoryBlocked ? 'pointer-events-none' : '' }}
                                     @if (in_array($i, $memoryFlipped) || in_array($i, $memoryMatched)) @if ($color === 'red') bg-red-500
...
bg-slate-300 hover:bg-slate-400
                                    ...
                                    @if (in_array($i, $memoryFlipped) || in_array($i, $memoryMatched))
                                        <span
                                            class="text-white font-bold text-sm">{{ strtoupper(substr($color, 0, 1)) }}</span>
                                    @else
                                        <i class="fas fa-question text-slate-600 dark:text-slate-400"></i>
                                    @endif
                                </div>
                            @endfor
                        </div>
                    ...
                    <div class="text-sm text-slate-600 dark:text-slate-400">Kliknite na kartice da ih okrenete i
                        pronađite parove!</div>
                </div>
            </div>
        @endif
        ...
        @if ($gameActive && $selectedGame === 'number_game')
            <div class="bg-white dark:bg-slate-800 rounded-lg shadow-lg p-2 md:p-6 mb-8">
                <div class="text-center">
                    ...
                    <div class="mb-6">
                        <div class="text-lg text-slate-600 dark:text-slate-400 mb-2">Ciljni broj:</div>
                        <div class="text-4xl font-bold text-purple-600 dark:text-purple-400 mb-4">{{ $numberTarget }}
                        </div>
                    ...
                    <div class="grid grid-cols-4 gap-3 max-w-md mx-auto">
                        <button wire:click="numberGameAction('add', 1)"
                            class="px-4 py-2 bg-sky-500 text-white rounded-lg hover:bg-sky-600">+1</button>
                        <button wire:click="numberGameAction('add', 5)"
                            class="px-4 py-2 bg-sky-500 text-white rounded-lg hover:bg-sky-600">+5</button>
                        <button wire:click="numberGameAction('add', 10)"
                            class="px-4 py-2 bg-sky-500 text-white rounded-lg hover:bg-sky-600">+10</button>
                        <button wire:click="numberGameAction('multiply', 2)"
                            class="px-4 py-2 bg-purple-500 text-white rounded-lg hover:bg-purple-600">×2</button>

                        <button wire:click="numberGameAction('subtract', 1)"
                            class="px-4 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600">-1</button>
                        <button wire:click="numberGameAction('subtract', 5)"
                            class="px-4 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600">-5</button>
                        <button wire:click="numberGameAction('subtract', 10)"
                            class="px-4 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600">-10</button>
                        <button wire:click="numberGameAction('divide', 2)"
                            class="px-4 py-2 bg-orange-500 text-white rounded-lg hover:bg-orange-600">÷2</button>
                    </div>

                    <div class="mt-4 text-sm text-slate-600 dark:text-slate-400">Koristite dugmad da dostignete ciljni
                        broj!</div>
                </div>
            </div>
        @endif
    ...
    <div class="bg-white dark:bg-slate-800 rounded-lg shadow-lg p-2 md:p-6 mb-8">
        <div class="flex items-center justify-between mb-6">
            ...
            <div class="text-sm text-slate-600 dark:text-slate-400">
                Bonus: {{ number_format(\App\Models\Setting::get('game_leaderboard_bonus', 50), 0, ',', '.') }} RSD za
                #1
            </div>
        </div>
                                ...
                                @foreach ($players->take(5) as $index => $player)
                                    <div
                                        ...
                                        <div class="flex items-center">
                                            ...
                                            <div>
                                                <div
                                                    class="font-medium text-slate-900 dark:text-slate-100 {{ auth()->id() === $player->user_id ? 'text-sky-600 dark:text-sky-400' : '' }}">
                                                    {{ $player->user->name }}
                                                    @if (auth()->id() === $player->user_id)
                                                        <span
                                                            class="text-xs text-sky-600 dark:text-sky-400">(Vi)</span>
                                                    @endif
                                                </div>
                                            </div>
                                        </div>
                                        <div
                                            class="font-bold {{ $index === 0 ? 'text-amber-600 dark:text-amber-400' : 'text-slate-600 dark:text-slate-400' }}">
            ...
            <div class="space-y-3">
                @foreach ($recentEarnings as $earning)
                    <div class="flex items-center justify-between p-3 bg-green-50 rounded-lg">
                        ...
                        <div
                            class="{{ str_contains($earning->type, 'leaderboard') ? 'text-amber-600 dark:text-amber-400' : 'text-green-600 dark:text-green-400' }} font-bold">
